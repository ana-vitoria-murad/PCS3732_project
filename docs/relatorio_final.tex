...
\newcommand{\caminho}[1]{\nolinkurl{#1}}
\newcommand{\VideoFinalURL}{\href{https://drive.google.com/file/d/1d5mtTm5FeTt85H9Ikr7ll8yA03pNou9N/view?usp=drive_link}{vídeo da demonstração comparativa no Google Drive}}
...
\begin{titlepage}
    ...
    Departamento de Engenharia de Computação e Sistemas Digitais\\[2cm]
...
\begin{figure}[H]
    \centering
    \resizebox{\textwidth}{!}{\input{docs/diagramas/arquitetura_fisica.tex}}
...
\begin{figure}[H]
    \centering
    \resizebox{\textwidth}{!}{\input{docs/diagramas/arquitetura_software.tex}}
    \caption{Arquitetura estática de software. Fonte: elaboração própria.}
    \label{fig:arquitetura-software}
\end{figure}

A interface mantém a visualização sincronizada com o estado do serviço por meio
da rota local \codigo{/api/state}. Como toda a comunicação ocorre na própria rede
do protótipo, esse mecanismo não depende de serviços externos nem altera o
caráter offline da solução.
...
\begin{figure}[H]
    \centering
    \resizebox{0.95\textwidth}{!}{\input{docs/diagramas/fluxo_reconhecimento.tex}}
...
As duas páginas em execução no hardware utilizado durante a apresentação são
mostradas na Figura~\ref{fig:interface-final}.

\begin{figure}[H]
    \centering
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{docs/imagens/interface_inicial.jpeg}\\
        \small (a) Página inicial.
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{docs/imagens/interface_reconhecimento.jpeg}\\
        \small (b) Resultado do reconhecimento.
    \end{minipage}
    \caption{Interface final em execução na Raspberry Pi 3. Fonte: elaboração própria.}
    \label{fig:interface-final}
\end{figure}

\section{Metodologia de desenvolvimento}

Adotou-se desenvolvimento incremental durante quatro ciclos semanais, com código
executável, documentação no README e histórico no Git. Não houve release na
primeira semana; foram preparadas duas releases intermediárias e uma release
final. O repositório público está disponível em
...
\begin{table}[H]
    ...
    \small
    \begin{tabularx}{\textwidth}{C{2.6cm} p{2.4cm} Y}
        \toprule
        \textbf{Ciclo} & \textbf{Foco} & \textbf{Entrega principal}\\
        \midrule
        \shortstack{Semana 1\\sem release} & Estruturação & Captura, inspeção de áudio,
        utilitários, espectrograma e banco inicial.\\
        \shortstack{Semana 2\\v0.1.0} & Base funcional & Refinamento do processamento,
        primeiros resultados e documentação.\\
        \shortstack{Semana 3\\v0.2.0} & Pipeline completo & Cadastro automático, novas
        músicas, matching e evidências experimentais.\\
        \shortstack{Semana 4\\release final} & Integração e robustez & Interface web, GPIO,
        cancelamento imediato, catálogo com 11 músicas e testes automatizados.\\
        \bottomrule
    \end{tabularx}
\end{table}
...
As evidências audiovisuais cobrem diferentes etapas do desenvolvimento: os dois
primeiros vídeos registram o pipeline inicial, os dois seguintes demonstram a
interface visual integrada e o último documenta a comparação entre gravações da
mesma composição.

\begin{itemize}
    \item \href{https://drive.google.com/file/d/1WVeXX_BBBwl9FQLqkDflyXSvk02ixFJV/view?usp=drive_link}{reconhecimento inicial de \emph{I Think We're Alone Now}};
    \item \href{https://drive.google.com/file/d/1dMfCgbtVWppEejYeSiKkwlpFuSDm9pZl/view?usp=drive_link}{reconhecimento inicial de \emph{I Can See You}};
    \item \href{https://drive.google.com/file/d/1rLpTn6fKmtMdpTGRO0p9CcuUjlkBJj4Z/view?usp=sharing}{reconhecimento de \emph{Faint} pela interface visual};
    \item \href{https://drive.google.com/file/d/1J5bIVzYEBFFINbpdiOmMa_muk8vZdySW/view?usp=sharing}{reconhecimento de \emph{I'm a Believer} pela interface visual}; e
    \item demonstração comparativa de \emph{I'm a Believer}, reunindo no mesmo
    vídeo a gravação cadastrada, o cover e a versão de outro artista:
    \VideoFinalURL.
\end{itemize}
...
ampliar o catálogo; e avaliar parâmetros de picos em ambientes e posições de
microfone variados.
...
\codigo{song-recognizer}. O ambiente virtual não faz parte da release: ele deve
ser recriado a partir do arquivo \codigo{requirements.txt}.
